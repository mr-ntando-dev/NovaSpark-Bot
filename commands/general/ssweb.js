...
module.exports = {
  ...
  async execute(sock, msg, args, extra) {
    const url = args[0];
    if (!url) return extra.reply('🌐 Provide a website URL!\n\n_Example: .ss https://google.com_');
    if (!url.startsWith('http')) return extra.reply('❌ URL must start with http:// or https://');
    ...
    try {
      ...
      await extra.reply(`❌ Screenshot failed: ${e.message}`);
    }
  },
};
